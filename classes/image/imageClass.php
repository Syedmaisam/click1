...
class imageClass extends Database
{
	...
	{
				if($param['check']=="" || $param['check']==NULL)
		{
			$overlaycheck=0;
		}
		else
		...
		$arrInsertCol = array('userId'=>$userId,'title'=>$param['inp_title'],'contentUrl'=>$param['inp_contentUrl'],'profile'=>$param["inp_Profile"],'imageLocation'=>$_FILES["inp_ImgLocation"]["name"],'imageUrl'=>$param['inp_ImgUrl'],'yourUrl'=>$param['inp_YourUrl'],'popupHeight'=>$param['inp_Height'],'popupWidth'=>$param['inp_Width'],'popupTiming'=>$param['inp_Timing'],'randomLink'=>$randomLink,'created'=>$currentDate,'popup_position'=>$param['position'],'overlay'=>$overlaycheck);
		$getresult_id = $this->insert(TABLE_TABLE_CONTENT,$arrInsertCol);
		$sql="select *,(Select profile_name from tbl_profile where id = profile) As ProfileName,(Select profile_image_path from tbl_profile where id = profile) As ProfileImage from tbl_imagecontent where id ='$getresult_id'";
	...
	public function editImageContent($param,$files='',$userId)
	{
		...
		$where = " id ='$userId'";
		$arrInsertCol = array('title'=>$param['inp_title'],'contentUrl'=>$param['inp_contentUrl'],'profile'=>$param["inp_Profile"],'imageLocation'=>$_FILES["inp_ImgLocation"]["name"],'imageUrl'=>$param['inp_ImgUrl'],'yourUrl'=>$param['inp_YourUrl'],'popupHeight'=>$param['inp_Height'],'popupWidth'=>$param['inp_Width'],'popupTiming'=>$param['inp_Timing'],'popup_position'=>$param['position']);
	...
	public function editImageContentwithOutImage($param,$userId)
	{
		...
		$where = " id ='$userId'";
		$arrInsertCol = array('title'=>$param['inp_title'],'contentUrl'=>$param['inp_contentUrl'],'profile'=>$param["inp_Profile"],'imageUrl'=>$param['inp_ImgUrl'],'yourUrl'=>$param['inp_YourUrl'],'popupHeight'=>$param['inp_Height'],'popupWidth'=>$param['inp_Width'],'popupTiming'=>$param['inp_Timing'],'popup_position'=>$param['position'],'overlay'=>$overlaycheck);
		return  $this->update(TABLE_TABLE_CONTENT, $arrInsertCol, $where);
	}
...
mysql_query("UPDATE tbl_imagecontent SET status=$status where id=$id");
