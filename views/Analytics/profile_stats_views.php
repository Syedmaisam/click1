...
if($analyticColl == null)
{
	$redirectlocation = SITE_ROOT_URL."views/settings/analytic_settings.php";
...
try {
	...
	$key_file_location = SOURCE_ROOT.GOOGLE_KEYFILE;
	...
	$cred = new Google_Auth_AssertionCredentials ( $Email_address, array (
			$scopes 
	), $key );
	...
	$Countryreport = $service->data_ga->get ( $trackid, $startdate, $enddate, "ga:pageviews", array (
	...
	$Socialreport = $service->data_ga->get ( $trackid, $startdate, $enddate, "ga:users", array (
	...
	$key = array_keys ( $Countryreport->getRows () );
	$country = '';
	$arrCountryData = '';
	...
	$duplCountryArry;
	foreach ( $Socialreport->getRows () as $row ) {
		
		// $serie_dim .= "'.$fechas[$i]."', ";
		// $arrSocialData = explode("~~",$Skey[$w]);
		$arrSocialData = $row [0];
		$isreferral = trim ( $arrSocialData, " " );
		if ($isreferral == "referral") {
			$socialName = trim ( $row [1], " " );
	...
	// ($country_pageview);
	// exit;
	$serie_dim = substr ( $serie_dim, 0, - 2 );
	$serie_val = substr ( $serie_val, 0, - 2 );
	
	$script1 = "<script type='text/javascript'src='http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js'></script><script type='text/javascript'>var chart;$(document).ready(function(){chart=new Highcharts.Chart({chart:{renderTo:'area-chart',defaultSeriesType:'area',marginRight:130,marginBottom:35},title:{text:'',x:-20},subtitle:{text:'Fuente: Google Analytics',x:-20},xAxis:{categories:[" . $serie_dim . "]},yAxis:{title:{text:''},plotLines:[{value:0,width:1,color:'#808080'}]},tooltip:{formatter:function(){return'<b>'+this.series.name+'</b><br/>'+
			this.x+': '+this.y+'';}},legend:{layout:'vertical',align:'right',verticalAlign:'top',x:-10,y:100,borderWidth:0},series:[{name:'Views',data:[" . $serie_val . "]},{name:'Visitors',data:[" . $sec . "]},{name:'Click',data:[" . $clk . "]}]});});</script>" . "<script type='text/javascript'src='http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js'></script><script type='text/javascript'src='../../grafico/highcharts.js'></script><script type='text/javascript'src='../../grafico/exporting.js'></script>" . "";
	
	$script2 = "<script type='text/javascript'src='http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js'></script><script type='text/javascript'>var chart;$(document).ready(function(){chart=new Highcharts.Chart({chart:{renderTo:'avgtime',defaultSeriesType:'line',marginRight:130,marginBottom:35},title:{text:'',x:-20},subtitle:{text:'Fuente: Google Analytics',x:-20},xAxis:{categories:[" . $serie_dim . "]},yAxis:{title:{text:''},plotLines:[{value:0,width:1,color:'#808080'}]},tooltip:{formatter:function(){return'<b>'+this.series.name+'</b><br/>'+
			this.x+': '+this.y+'';}},legend:{layout:'vertical',align:'right',verticalAlign:'top',x:-10,y:100,borderWidth:0},series:[{name:'Views',data:[" . $timeonpageinmin . "]}]});});</script>" . "<script type='text/javascript'src='http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js'></script><script type='text/javascript'src='../../grafico/highcharts.js'></script><script type='text/javascript'src='../../grafico/exporting.js'></script>" . "";
} catch ( Exception $e ) {
	//var_dump($e); exit;
}
...
<link rel="stylesheet" type="text/css"
	href="<?php echo SITE_ROOT_URL; ?>css/datepickr.css" />

<script type="text/javascript"
	src="<?php echo SITE_ROOT_URL; ?>js/datepickr.js"></script>
<meta charset="UTF-8">
	...
	<div class="wrapper row-offcanvas row-offcanvas-left">
						...
						<form id="dateForm" class="form-bordered"
							...
							<div class="panel-body">
								...
								<div class="input-group date">
									<input type="text" value="<?php echo $startdate; ?>"
										id="datepickerstart" placeholder="yy-mm-dd" name="fromDate"
								...
								<div class="input-group date">
									<input type="text" value="<?php echo $enddate; ?>"
										id="datepickerend" placeholder="yy-mm-dd" name="toDate"
										class="form-control hasDatepicker"><span
										class="input-group-addon"><button type="button"
											class="ui-datepicker-trigger">
											<i class="fa fa-calendar"></i>
      ...
      function drawChart() {
        ...
        chart.draw(data, options);
					...
					<div style="display: none;" class="panel panel-default panel-stat-header">
						<!--  top referrels --------------------------  -->
						<div class="panel-heading">
...
function makehrefDate()
{
	...
	var href = SITE_ROOT_URL+"views/Analytics/profile_stats_views.php?id="+ids+"&fromDate="+startdate+"&toDate="+enddate;
