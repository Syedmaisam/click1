...
$arrLinkData=$objBasic->getFormPollDetailsByRandomLink($arrUrl[1]);

echo "<div id='Customhtmllink' style='display:none;'>".html_entity_decode($arrLinkData[0]['Custom_Html'])."</div>";
...
echo "<meta property='og:url' content='".$url."' /><iframe src='".$url."' sandbox='allow-forms allow-scripts' style='height:99%;width:100%;border:none;margin:0;padding:0'></iframe>";
...
//}
?>
<div><input id="randomurl" type="hidden" value="<?php echo $arrUrl[1]; ?>"></div>
<div><input id="typeid" type="hidden" value="<?php echo $linkData['id']; ?>"></div>
<div><input id="TypeInfo" type="hidden" value="<?php echo $arrLinkData[0]['Type']; ?>"></div>
...
<div style="display:none;"><?php echo html_entity_decode($arrLinkData[0]['Custom_Html']); ?></div>
...
var customhtmldata=$("#Customhtmllink").html();
var sub_name=$("#TypeInfo").val();



$(document).on('keyup', '#INP_Form', function() {
		...
		$("input[name='fields_fname']").val(value);
		$("input[name='fields[name]']").val(value);
	});

$(document).on('keyup', '#INP_EmailForm', function() {
	
		var value=$("#INP_EmailForm").val();
		$("input[name='email']").val(value);
		$("input[name='EMAIL']").val(value);
		$("input[name='fields[email]']").val(value);
		$("input[name='fields_email']").val(value);
...
$("html body").css("overflow","hidden");
	...
	{ 
		...
		if(sub_name=='Forms')
		{
			...
			var valueform=$("#INP_Form").val();
			var valueEmail=$("#INP_EmailForm").val();
			if(valueform==''&& valueEmail=='')
			{
				//alert("Please fill Name and Email_Id");
                               $('#INP_Form').css('border','solid red');
				$('#INP_EmailForm').css('border','solid red');				
                               return false;
			}
			else if(valueform!=''&& valueEmail=='')
			{
			...
			else if(valueform==''&& valueEmail!='')
			{
				//alert("Please fill Name");
                                $('#INP_Form').css('border','solid red');
				$('#INP_EmailForm').css('border','');
				return false;
			}
                        $('#INP_Form').css('border','');
			$('#INP_EmailForm').css('border','');
			var randomurl= $("#randomurl").val();
                        var ans = $("#questionsList").val();
			  $.ajax({          	
	      			url:SITE_ROOT_URL+'ajax/ipaddress/AnswerCount.php',
	             	type:"post",
	      		 	data: {
	      					    submit: "submit",
	      					    tablename: "tbl_formsandpoll",
		...
		if ( $( "#questionsList" ).length ) {
			if($( "#questionsList" ).val() == "Please choose one of answers")
			{
			 alert("Please Choose One of the Answer");
			 return false;
			}
		 
		}
		else if($( "#inpmsg" ).length>0)
		{
			if($( "#inpmsg" ).val() == '')
			{
...
ans:ans,
      		     ...
      		     success: function(response) { 
              	       var linkbgcl= $('#linkbgcl').attr('href');
       				   $('#linkbgcl').attr('target','_parent');
      				   window.location.replace(linkbgcl); 
      				   return false; 		   
      			}	
      		});	
